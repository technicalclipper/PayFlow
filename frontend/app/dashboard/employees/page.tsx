          ...
          <GlassCard key={employee.id} hoverable className="p-5" transition={{ delay: i * 0.04, duration: 0.4 }}>
            ...
            <div className="space-y-2.5">
              ...
              <div className="flex items-center gap-2.5 text-sm text-text-secondary">
                ...
                <code className="text-xs bg-white/5 px-2 py-0.5 rounded truncate">{employee.walletId}</code>
              </div>
            </div>
            <div className="mt-4 pt-3 border-t border-glass-border">
              <p className="text-xs text-text-muted">
                Added {new Date(employee.createdAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}
              </p>
            </div>
          </GlassCard>
        ))}
