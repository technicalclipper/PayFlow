    ...
    <div className="space-y-8">
      <div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <motion.h1 initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="text-2xl sm:text-3xl font-bold tracking-tight flex items-center gap-3">
            Payroll — {new Date(payroll.payoutDate).toLocaleDateString("en-US", { month: "long", year: "numeric" })}
            <Badge status={payroll.status} />
          </motion.h1>
          <div className="flex gap-3">
            <Button variant="secondary" size="sm" icon={<Send className="w-4 h-4" />}>Distribute</Button>
            <Button variant="secondary" size="sm" icon={<FileText className="w-4 h-4" />}>Generate PDFs</Button>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <GlassCard className="p-5">
          ...
          <p className="text-2xl font-bold">{payroll.totalAmount.toLocaleString()} <span className="text-sm font-normal text-text-muted">USDC</span></p>
        </GlassCard>
        <GlassCard className="p-5">
          <div className="flex items-center gap-3 mb-2">
          ...
          <p className="text-2xl font-bold">{new Date(payroll.payoutDate).toLocaleDateString("en-US", { month: "short", day: "numeric" })}</p>
        </GlassCard>
        <GlassCard className="p-5">
          <div className="flex items-center gap-3 mb-2">
          ...
          <p className="text-2xl font-bold">{payroll.employees.filter((e) => e.status === "redeemed").length}<span className="text-sm font-normal text-text-muted"> / {payroll.employees.length}</span></p>
        </GlassCard>
      </div>
                ...
                <motion.tr key={pe.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.05 + i * 0.03 }} className="border-b border-glass-border last:border-0 hover:bg-white/[0.02] transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full gradient-bg flex items-center justify-center text-xs font-bold text-white">{pe.employee.name.split(" ").map((n) => n[0]).join("")}</div>
                      <div>
                  ...
                  <td className="px-6 py-4"><code className="text-xs text-text-muted bg-white/5 px-2 py-1 rounded-md">{pe.employee.walletId}</code></td>
                  <td className="px-6 py-4"><span className="text-sm font-semibold">{pe.salaryAmount.toLocaleString()} USDC</span></td>
                  <td className="px-6 py-4"><Badge status={pe.status} /></td>
                  <td className="px-6 py-4">{pe.status === "funded" && <Link href={`/salary-slip/${pe.id}`}><Button variant="ghost" size="sm">View Slip</Button></Link>}</td>
                </motion.tr>
              ))}
            </tbody>
          </table>
