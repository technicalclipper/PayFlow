...
import { Zap, Wallet, ArrowRight, Copy, Check, ExternalLink, Shield, Clock, AlertTriangle } from "lucide-react";
import Button from "@/components/ui/Button";
import Input from "@/components/ui/Input";
import AnimatedCheckmark from "@/components/ui/AnimatedCheckmark";
import AnimatedCounter from "@/components/ui/AnimatedCounter";
import { claimSalary } from "@/lib/actions";
...
export default function RedeemClient({
  ...
  const isClaimable = (status === "funded" && !isExpired && !isUsed) || isDemo;
  const isAlreadyRedeemed = status === "redeemed" || isUsed;
  ...
  const handleClaim = useCallback(async () => {
    ...
    if (isDemo) {
      ...
      setClaimedAmount(salaryAmount);
      setState("success");
      setConfettiActive(true);
      return;
    }

    const result = await claimSalary(token, walletAddress);
    if (result.error) {
      setError(result.error);
      setState("error");
    } else {
      setClaimedAmount(Number(result.amount));
      setState("success");
      setConfettiActive(true);
    }
  }, [walletAddress, token, isDemo, salaryAmount]);
      ...
      <motion.div initial={{ opacity: 0, y: 24, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.6, ease: [0.4, 0, 0.2, 1] }} className="w-full max-w-md relative z-10">
        <div className="bg-[#0f1420]/90 backdrop-blur-2xl border border-white/[0.08] rounded-2xl overflow-hidden shadow-2xl shadow-black/50">
          ...
          <div className="px-6 pt-6 pb-0 flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-[10px] text-text-muted">Salary Voucher {isDemo && "• Demo"}</p>
              </div>
            </div>
          ...
          <AnimatePresence mode="wait">
            {/* Error / Already Redeemed state */}
            {(errorMessage || isAlreadyRedeemed || (isExpired && !isDemo)) ? (
              <motion.div key="error-state" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-6 sm:p-8 text-center">
                <div className="w-16 h-16 rounded-2xl bg-error/10 flex items-center justify-center mx-auto mb-4">
                  <AlertTriangle className="w-8 h-8 text-error" />
                </div>
                <h2 className="text-lg font-bold mb-2">
                  {isAlreadyRedeemed ? "Already Redeemed" : isExpired ? "Token Expired" : "Invalid Link"}
                </h2>
                <p className="text-sm text-text-secondary">
                  {isAlreadyRedeemed ? "This salary has already been claimed." : isExpired ? "This redeem link has expired." : errorMessage}
                </p>
              </motion.div>
            ) : state !== "success" ? (
              <motion.div key="claim-form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }} className="p-6 sm:p-8">
                <div className="text-center mb-8">
                  <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 0.2, duration: 0.5 }}>
                    <p className="text-xs uppercase tracking-widest text-text-muted mb-1">{employeeName}</p>
                    <div className="text-6xl sm:text-7xl font-bold tracking-tighter my-3">
                      <AnimatedCounter end={salaryAmount} duration={2} className="gradient-text" />
                    </div>
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="space-y-4">
                  ...
                  <Button size="lg" className="w-full" onClick={handleClaim} loading={state === "loading"} disabled={!walletAddress || !isClaimable} icon={state !== "loading" ? <ArrowRight className="w-5 h-5" /> : undefined}>
                    {state === "loading" ? "Processing Transfer..." : "Claim Salary"}
                  </Button>
                  <Button variant="secondary" size="md" className="w-full"><Wallet className="w-4 h-4 mr-2" /> Use my wallet</Button>
                </motion.div>
              ...
              <motion.div key="success" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5, ease: [0.4, 0, 0.2, 1] }} className="p-6 sm:p-8 text-center">
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}>
                  ...
                  <p className="text-sm text-text-secondary mb-6">{(claimedAmount || salaryAmount).toLocaleString()} USDC has been sent to your wallet</p>
                  <div className="bg-white/[0.03] rounded-xl p-4 mb-6 space-y-3 text-left">
                    <div className="flex justify-between text-sm">
                      <span className="text-text-secondary">Amount</span>
                      <span className="font-semibold text-success">{(claimedAmount || salaryAmount).toLocaleString()} USDC</span>
                    </div>
                    <div className="flex justify-between text-sm">
